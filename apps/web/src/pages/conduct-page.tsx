...
const message = (reason: unknown) => apiErrorMessage(reason, 'Conduct data could not be loaded.')
...
export function ConductPage({ client }: { client: ConductApiClient }) {
  ...
  useEffect(() => {
    ...
    }, (reason: unknown) => { if (active) { setPageError(message(reason)); setLoading(false) } })
    return () => { active = false }
  }, [client])

  async function action(key: string, operation: () => Promise<unknown>, success: string, refreshFailure = 'Changes were saved, but the latest conduct data could not be refreshed. Retry refresh.') {
    const result = await runMutationWithRefresh(key, operation, load)
    if (result.status === 'success') { notify(success, 'success'); return true }
    if (result.status === 'refresh-failed') { setPageError(refreshFailure); notify(refreshFailure, 'warning'); return true }
    if (result.status === 'mutation-failed') notify(apiErrorMessage(result.error, 'The conduct action could not be completed.'), 'error')
    return false
  }
  ...
  function respondDecline(dutyId: string) {
    ...
    if (reason) void action(`decline-${dutyId}`, () => client.decline(dutyId, { reason }), 'Duty declined. The controller can assign a replacement.')
  }

  function reopen(current: ConductSittingRecord) {
    const reason = window.prompt('Reason for reopening submitted attendance')?.trim()
    if (reason) void action(`reopen-${current.id}`, () => client.reopenAttendance(current.id, { expectedVersion: current.attendance.version, reason }), 'Attendance reopened with an audit reason.')
  }
  ...
  function dispose(incidentId: string, expectedVersion: number, disposition: 'CLEARED' | 'RETAIN_WITHHELD' | 'NO_RESULT_IMPACT') {
    ...
    if (reason) void action(`dispose-${incidentId}-${disposition}`, () => client.disposeIncident(incidentId, { disposition, expectedVersion, reason }), disposition === 'RETAIN_WITHHELD' ? 'WITHHELD retained.' : 'Incident disposition saved.')
  }
      ...
      {sitting ? <div className="conduct-stack">
        <section className="conduct-card"><header><div><p className="eyebrow">{sitting.examCode}</p><h2>{sitting.subjectCode} · {sitting.subjectName}</h2><p>{sitting.hallCode} · {sitting.hallName} · {displayTime(sitting.startsAt, sitting.timezone)} – {new Intl.DateTimeFormat('en-IN', { timeZone: sitting.timezone, timeStyle: 'short' }).format(new Date(sitting.endsAt))}</p></div><span className={`status-badge ${sitting.attendance.state === 'SUBMITTED' ? 'active' : ''}`}>{sitting.attendance.state}</span></header>
          <div className="duty-panel"><div><h3>Assigned duty</h3>{sitting.duties.length ? sitting.duties.map((duty) => <p key={duty.id}><b>{duty.facultyName}</b> <span className={`status-badge ${duty.state === 'ACCEPTED' ? 'active' : duty.state === 'DECLINED' ? 'inactive' : ''}`}>{duty.state}</span>{duty.declineReason ? <small>{duty.declineReason}</small> : null}</p>) : <p>No duty assigned.</p>}</div>
            {controller ? <div className="duty-assignment"><label>Faculty<select value={facultyId} onChange={(event) => setFacultyId(event.target.value)}>{snapshot.faculty.map((faculty) => <option value={faculty.id} key={faculty.id}>{faculty.name} · {faculty.departmentName}</option>)}</select></label><AsyncButton className="primary-button" disabled={isPending || !facultyId} loading={pendingAction === `assign-${sitting.id}`} loadingText="Assigning…" onClick={() => void action(`assign-${sitting.id}`, () => client.assign(sitting.id, { facultyId, ...(declinedDuty ? { replacesDutyId: declinedDuty.id } : {}) }), declinedDuty ? 'Replacement duty assigned.' : 'Duty assigned.')}>{declinedDuty ? 'Assign replacement' : 'Assign duty'}</AsyncButton></div> : ownDuty?.state === 'PENDING' ? <div className="duty-actions"><AsyncButton className="primary-button" disabled={isPending} loading={pendingAction === `accept-${ownDuty.id}`} loadingText="Accepting…" onClick={() => void action(`accept-${ownDuty.id}`, () => client.accept(ownDuty.id), 'Duty accepted. Roster access is now enabled.')}>Accept</AsyncButton><AsyncButton className="secondary-button" disabled={isPending} loading={pendingAction === `decline-${ownDuty.id}`} loadingText="Declining…" onClick={() => respondDecline(ownDuty.id)}>Decline</AsyncButton></div> : null}
          </div>
        </section>
        ...
        <section className="conduct-card"><header><div><h2>Hall roster</h2><p>{sitting.attendance.rows.length} allocated students · {notMarked} not marked</p></div></header>
          {sitting.attendance.rows.length === 0 ? <p className="conduct-locked">Accept the assigned duty to access this roster.</p> : <><div className="attendance-table-wrap"><table className="attendance-table"><thead><tr><th>Seat</th><th>Student</th><th>Attendance</th></tr></thead><tbody>{sitting.attendance.rows.map((row) => <tr key={row.seatAssignmentId}><td>{String(row.seatNumber).padStart(2, '0')}</td><td><b>{row.studentName}</b><small>{row.rollNo}</small></td><td><select aria-label={`Attendance for ${row.studentName}`} disabled={isPending || !sitting.canEditAttendance} value={draft[row.seatAssignmentId] ?? row.state} onChange={(event) => setAttendanceDrafts((current) => ({ ...current, [sitting.id]: { ...draft, [row.seatAssignmentId]: event.target.value as AttendanceState } }))}>{attendanceStates.map((state) => <option value={state} key={state}>{state.replace('_', ' ')}</option>)}</select></td></tr>)}</tbody></table></div>
            <footer className="conduct-actions"><span>{sitting.attendance.state === 'SUBMITTED' ? `Submitted ${displayTime(sitting.attendance.submittedAt!, sitting.timezone)}` : `${notMarked} rows block submission`}</span>{controller && sitting.attendance.state === 'SUBMITTED' ? <AsyncButton className="secondary-button" disabled={isPending} loading={pendingAction === `reopen-${sitting.id}`} loadingText="Reopening…" onClick={() => reopen(sitting)}>Reopen with reason</AsyncButton> : <><AsyncButton className="secondary-button" disabled={isPending || !sitting.canEditAttendance} loading={pendingAction === `save-attendance-${sitting.id}`} loadingText="Saving…" onClick={() => void action(`save-attendance-${sitting.id}`, () => client.saveAttendance(sitting.id, { expectedVersion: sitting.attendance.version, rows: sitting.attendance.rows.map((row) => ({ seatAssignmentId: row.seatAssignmentId, state: draft[row.seatAssignmentId] ?? row.state })) }), 'Attendance draft saved.')}>Save draft</AsyncButton><AsyncButton className="primary-button" disabled={isPending || !sitting.canEditAttendance || notMarked > 0 || sitting.attendance.version === 0} loading={pendingAction === `submit-attendance-${sitting.id}`} loadingText="Submitting…" onClick={() => void action(`submit-attendance-${sitting.id}`, () => client.submitAttendance(sitting.id, { expectedVersion: sitting.attendance.version }), 'Attendance submitted and locked.', 'Attendance was submitted, but the latest attendance state could not be reloaded. Retry refresh.')}>Submit attendance</AsyncButton></>}</footer>
          </>}
        </section>
