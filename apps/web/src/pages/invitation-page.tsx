...
export function InvitationPage({ client }: { client: AuthApiClient }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(reason instanceof AuthApiError ? reason.message : 'Invitation acceptance failed')
    }
  }
  return (
    <AuthLayout title="Accept invitation" intro="Confirm this one-time invitation to activate your institution membership.">
      {complete ? (
        ...
        <form onSubmit={submit} className="auth-form">
          ...
          <button className="primary-button" disabled={!token}>Accept invitation</button>
        </form>
      )}
    </AuthLayout>
  )
