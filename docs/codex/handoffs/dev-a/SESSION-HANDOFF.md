...
Base: `e5f2250` (B01 merged integration) plus rebased A01/A02 commits
Head: uncommitted A03 working tree
...
- Developer A retains migration and shared-contract locks through A03 merge.
- After merge, transfer/release both locks in `CURRENT-STATE.md`.
...
1. If credentials are available, verify Northstar student save/submit and
   controller reject/resubmit/approve plus Cedar auto-enrol at `/exams`.
2. Review diff, commit A03, merge through `integration`.
3. Release/transfer migration and contract locks.
4. A04/B02 pull integration and consume approved `RegistrationSubject.id` rows.
