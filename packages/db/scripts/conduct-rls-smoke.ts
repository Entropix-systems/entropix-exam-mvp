...
try {
  ...
  const currentExam = { hallSitting: { examPaper: { exam: { code: 'ANNUAL-2026' } } } };
  const cedarState = await withTenant(prisma, cedar.id, async (tx) => ({
    duties: await tx.duty.count({ where: currentExam }),
    active: await tx.duty.count({ where: { ...currentExam, state: { in: ['PENDING', 'ACCEPTED'] } } }),
  ...
  if (cedarState.duties !== 3 || cedarState.active !== 3 || cedarState.invigilators < 3 || cedarState.sittings !== 3) throw new Error(`Cedar conduct fixture is invalid: ${JSON.stringify(cedarState)}`);
  ...
  const cedarDutyId = await withTenant(prisma, cedar.id, async (tx) => (await tx.duty.findFirstOrThrow({ where: currentExam })).id);
